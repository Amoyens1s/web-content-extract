...
function extractSeoMetadata(document: Document): SeoMetadata {
  ...
  metadata.siteName =
    document
      ...
      ?.getAttribute("content") ||
    document.querySelector('[itemprop="publisher"]')?.getAttribute("content") ||
    undefined;
  ...
  if (
    metadata.openGraph &&
    Object.values(metadata.openGraph).every((value) => !value)
  ) {
...
async function fetchHtml(url: string): Promise<string> {
  try {
    ...
    if (error instanceof Error && error.name === "TimeoutError") {
      throw new Error(
        `Request timeout: Failed to fetch ${url} within 10 seconds.`,
      );
    }
